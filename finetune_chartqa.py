...
def main():
    ...
    parser.add_argument('--data-path', type=str, default="ahmed-masry/chartqa_without_images",
                        help='Path to the data file')
    parser.add_argument('--train-images', type=str, default='/content/ChartQA Dataset/train/png/',
                        help='Path to the training images')
    parser.add_argument('--valid-images', type=str, default='/content/ChartQA Dataset/val/png',
                        help='Path to the validation images')
    parser.add_argument('--test-images', type=str, default='/content/ChartQA Dataset/test/png',
                        help='Path to the validation images')

    parser.add_argument('--output-dir', type=str, default="/content/output_data",
                        help='Path to the output directory for saving the checkpoints')
    ...
    # parser.add_argument('--checkpoint-path', type=str, default="ahmed-masry/unichart-base-960",
    ...
    train_dataset = ChartQADataset(dataset["train"], images_folder=args.train_images, processor=processor,
                                   max_length=args.max_length,
                                   split="train", prompt_end_token="<s_answer>", task_prefix="<chartqa>"
                                   )

    val_dataset = ChartQADataset(dataset["val"], images_folder=args.valid_images, processor=processor,
                                 max_length=args.max_length,
                                 split="valid", prompt_end_token="<s_answer>", task_prefix="<chartqa>"
                                 )

    _dataset = ChartQADataset(dataset["test"], images_folder=args.valid_images, processor=processor,
                                 max_length=args.max_length,
                                 split="test", prompt_end_token="<s_answer>", task_prefix="<chartqa>"
                                 )
    ...
    checkpoint_callback = ModelCheckpoint(dirpath=args.output_dir, every_n_train_steps=args.checkpoint_steps,
